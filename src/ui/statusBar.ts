...
export class StatusBarManager {
    ...
    updateStatus(status: ConnectionStatus, details?: {
        clientCount?: number;
        port?: number;
        error?: string;
    }): void {
    ...
    private getStatusDisplay(): { text: string; tooltip: string; color?: string } {
    ...
    async showStatusDetails(): Promise<void> {
        ...
        if (this.currentStatus === ConnectionStatus.Connected && this.clientCount === 0) {
            options.push({
    ...
    private async handleStatusAction(action: string): Promise<void> {
        if (action.includes('Start Monitoring')) {
            ...
            await vscode.commands.executeCommand('workbench.action.openSettings', 'angularXray');
        } else if (action.includes('Show Output Log')) {
            await vscode.commands.executeCommand('workbench.action.output.toggleOutput', 'Angular X-Ray');
    ...
    private async showConnectionDetails(): Promise<void> {
        const details = [
            ...
            `**WebSocket Server:** ${this.currentStatus === ConnectionStatus.Connected ? 'Running on port 3333' : 'Not running'}`,
            `**Connected Clients:** ${this.clientCount}`,
            `**Data Points Received:** ${this.dataCount}`,
            ''
        ];
